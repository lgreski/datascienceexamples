# ToothGrowth Analysis: Full Factorial ANOVA
Len Greski  
November 25, 2015  
...
* One way analysis of variance on the independent variables produces results consistent with t-tests to compare group means within an effect, but 
* Analysis of variance allowed us to identify a main effect for supplement type that was  suppressed by dosage. 
...
Next, Kabacoff reports the means and standard deviations for each cell in the design. 
...
Note that the above chart does not account for the fact that we're conducting multiple comparisons of means, which increases our likelihood of a type 1 error: rejecting the null hypothesis when it is in fact true. Since we have 6 means in the study, we can make up to $\binom {6}{2} = 15$ comparisons of means, and the probability of at least one type 1 error among 15 comparisons is 0.537, or almost 54% [(1)](https://en.wikipedia.org/wiki/Multiple_comparisons_problem). Due to the multiple comparisons problem, analysis of variance is the preferred technique with with to analyze this data. 
...
Analysis of Variance is particularly well suited for analyses that are organized with an experimental design: placing subjects randomly into specific categories of test and control groups, to test the effectiveness of a "test" treatment versus a control group. For a more detailed explanation of the various ANOVA models, the reader is referred to *R in Action*, pp. 212 - 238. 

As stated earlier, the ToothGrowth data set is used to illustrate the concepts of two-way factorial ANOVA, where the dependent variable is assessed across groups formed by the combination of two factor variables, `dose` and `supp`. There are two major benefits of the analysis of variance over paired comparisons of means, including:
...
Finally, we report the two-way factorial ANOVA test along with the model coefficients, as presented in *R in Action*. When given a model forumla is specified as an interaction effect, the `aov()` function automatically includes the relevant main effects in the model. 
...
Mean <- c(13.23, 13.23 - 5.25, 13.23 + 9.47, 13.23 + 9.47  - 5.25 - 0.68,13.23 + 12.83,
...
The entire analysis may be summarized visually with the `interaction2wt()` plot that Kabacoff uses in closing to summarize the analysis. 
...
![](ToothGrowthFactorialANOVA_files/figure-html/int2way-1.png) 

# Conclusions

The Analysis of Variance (ANOVA) techinque is very useful for studies that are organized using experimental designs that include test and control groups. We've compared the analysis to a series of hypothesis tests / comparisons of means, and illustrated the value of accounting for the interactions between factor variables. In the case of the ToothGrowth data set, a main effect that was not significantly different from zero when analyzed by itself became significant after we accounted for a third variable. This is known as a suppression effect, where the relationship between Y and X is "suppressed" by a third variable [(3)](http://stats.stackexchange.com/questions/73869/suppression-effect-in-regression-definition-and-visual-explanation-depiction). 

# Endnotes

[(1)](https://en.wikipedia.org/wiki/Multiple_comparisons_problem) Multiple Comparisons Problem, Wikipedia, retrieved November 25, 2015. 

[(2)](https://en.wikipedia.org/wiki/F-test) F-test, Wikipedia, retrieved November 25, 2015.

[(3)](http://stats.stackexchange.com/questions/73869/suppression-effect-in-regression-definition-and-visual-explanation-depiction) Suppression Effect in Regression, Cross Validated Stack Exchange Website, retrieved November 25, 2015. 
